...
export const processEngineeringAI = onCall(
  ...
  async (request) => {
    ...
    const { remaining } = await spendWalletTokens(uid, toolConfig.tokenCost, `engineering_ai_${tool}`);
    ...
    try {
      ...
      await refundWalletTokens(uid, toolConfig.tokenCost, `engineering_ai_${tool}`).catch(() => undefined);
      throw error;
    }
  }
);
